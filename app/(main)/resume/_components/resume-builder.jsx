...
export default function ResumeBuilder({ initialContent }) {
  ...
  //   if (!element) {
  //     toast.error("Failed to capture PDF content.");
  //     return;
  //   }
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // Generate image from the element
      const imgData = await domtoimage.toPng(element, {
        quality: 0.98,
        width: element.clientWidth * scaleFactor,
        height: element.clientHeight * scaleFactor,
      ...
      // Scale image dimensions to fit inside the PDF with margins
      const imgWidth = pageWidth - 2 * margin;
      const imgHeight = (element.clientHeight * imgWidth) / element.clientWidth;
  
      let y = margin;
  
      // Add image and apply margins
      if (imgHeight < pageHeight - 2 * margin) {
        pdf.addImage(imgData, "PNG", margin, y, imgWidth, imgHeight);
      } else {
        // Handle multi-page PDF if content is large
...
//           <div className="border rounded-lg">
//             <div ref={pdfRef} id="resume-pdf" className="p-6 bg-white text-black">
